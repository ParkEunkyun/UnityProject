...
public class RealBossMob : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        enemyHP.value = maxHP;
        HP.text = enemyHP.maxValue.ToString();
    ...
    private void Update()
    {
        ...
        HP.text = enemyHP.value.ToString();
    ...
    public ItemSO itemDrop3;
    public ItemSO itemDrop4;
    public ItemSO itemDrop5;
    public ItemSO itemDrop6;
    ...
    void DropItems3() // 고급재료
    {
        for (int i = 0; i < dropAmount; i++)
        {
            ...
            Vector3 force = new Vector3(Random.Range(-2f, 2f), 2, Random.Range(-2f, 2f));
            ItemPickupable drop = (Instantiate(Resources.Load("Item Pickupable 1"), transform.position + (force / 4f), Quaternion.identity) as GameObject).GetComponent<ItemPickupable>();
            drop.SetUpPickupable(itemDrop3, 1);
            drop.GetComponent<Rigidbody2D>().AddForce(force, ForceMode2D.Impulse);
        }
    }
    void DropItems4() // 고급재료
    {
        for (int i = 0; i < dropAmount; i++)
        {
            //Spawn force and position. Random so they all pop out in different directions
            Vector3 force = new Vector3(Random.Range(-2f, 2f), 2, Random.Range(-2f, 2f));
            ItemPickupable drop = (Instantiate(Resources.Load("Item Pickupable 1"), transform.position + (force / 4f), Quaternion.identity) as GameObject).GetComponent<ItemPickupable>();
            drop.SetUpPickupable(itemDrop4, 1);
            drop.GetComponent<Rigidbody2D>().AddForce(force, ForceMode2D.Impulse);
        }
    }
    void DropItems5() // 고급재료
    {
        for (int i = 0; i < dropAmount; i++)
        {
            //Spawn force and position. Random so they all pop out in different directions
            Vector3 force = new Vector3(Random.Range(-2f, 2f), 2, Random.Range(-2f, 2f));
            ItemPickupable drop = (Instantiate(Resources.Load("Item Pickupable 1"), transform.position + (force / 4f), Quaternion.identity) as GameObject).GetComponent<ItemPickupable>();
            drop.SetUpPickupable(itemDrop5, 1);
            drop.GetComponent<Rigidbody2D>().AddForce(force, ForceMode2D.Impulse);
        }
    }
    void DropItems6() // 고급재료
    {
        for (int i = 0; i < dropAmount; i++)
        {
            //Spawn force and position. Random so they all pop out in different directions
            Vector3 force = new Vector3(Random.Range(-2f, 2f), 2, Random.Range(-2f, 2f));
            ItemPickupable drop = (Instantiate(Resources.Load("Item Pickupable 1"), transform.position + (force / 4f), Quaternion.identity) as GameObject).GetComponent<ItemPickupable>();
            drop.SetUpPickupable(itemDrop6, 1);
            drop.GetComponent<Rigidbody2D>().AddForce(force, ForceMode2D.Impulse);
        }
    }
}
